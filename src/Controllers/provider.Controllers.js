...
const register=async(req,res)=>{
    try{
        ...
        const newUser=await providerModel.insertOne({...req.body,password:hashPassword});
...
const login= async (req, res) => {
    try {
        ...
        const isPasswordValid = await bcrypt.compare(req.body.password+secret, user.password);
        if (!isPasswordValid) {
            return res.status(401).json({ reason: "Invalid credentials" });
        }
